...
executor.update_parameters(timeout_min=10, slurm_partition="microcloud")
...
with executor.batch():
    for config_folders in os.listdir(config_folder):
        job = executor.submit(train_data.avg_trj, config_folders)
        jobs.append(job)
#Combining data sets
#files = train_data.make_folder_list("2d_lin_time_file_list_corr.txt")
